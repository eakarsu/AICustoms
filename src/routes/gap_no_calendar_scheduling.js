/**
 * Gap Feature: gap-no-calendar-scheduling
 * No calendar / scheduling.
 *
 * POST /api/gap-no-calendar-scheduling
 * Auth required. Generated as part of Gaps backend scaffold (batch_02).
 * Integration credentials: process.env.FEATURE_GAP_NO_CALENDAR_SCHEDULING_KEY
...
const FEATURE_KEY = process.env.FEATURE_GAP_NO_CALENDAR_SCHEDULING_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: No calendar / scheduling..\nGoal: No calendar / scheduling.\nRespond with clear, actionable analysis. Prefer JSON when structured output is requested.`;
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AICustoms - gap-no-calendar-scheduling',
    },
    body: JSON.stringify({
...
router.post('/', auth, async (req, res) => {
  try {
    ...
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_GAP_NO_CALENDAR_SCHEDULING_KEY');
    const ai = await callLLM({ feature: 'gap-no-calendar-scheduling', goal: 'No calendar / scheduling.', input: payload });
    try {
      ...
      if (pool && pool.query) {
        await pool.query(
          ...
          ['gap-no-calendar-scheduling', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[gap-no-calendar-scheduling] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
      feature: 'gap-no-calendar-scheduling',
      endpoint: '/api/gap-no-calendar-scheduling',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
    ...
    console.error('[gap-no-calendar-scheduling] error:', err.message);
