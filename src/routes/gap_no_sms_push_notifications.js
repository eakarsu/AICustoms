/**
 * Gap Feature: gap-no-sms-push-notifications
 * No SMS / push notifications.
 *
 * POST /api/gap-no-sms-push-notifications
 * Auth required. Generated as part of Gaps backend scaffold (batch_02).
 * Integration credentials: process.env.FEATURE_GAP_NO_SMS_PUSH_NOTIFICATIONS_KEY
...
const FEATURE_KEY = process.env.FEATURE_GAP_NO_SMS_PUSH_NOTIFICATIONS_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: No SMS / push notifications..\nGoal: No SMS / push notifications.\nRespond with clear, actionable analysis. Prefer JSON when structured output is requested.`;
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AICustoms - gap-no-sms-push-notifications',
    },
    body: JSON.stringify({
...
router.post('/', auth, async (req, res) => {
  try {
    ...
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_GAP_NO_SMS_PUSH_NOTIFICATIONS_KEY');
    const ai = await callLLM({ feature: 'gap-no-sms-push-notifications', goal: 'No SMS / push notifications.', input: payload });
    try {
      ...
      if (pool && pool.query) {
        await pool.query(
          ...
          ['gap-no-sms-push-notifications', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[gap-no-sms-push-notifications] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
      feature: 'gap-no-sms-push-notifications',
      endpoint: '/api/gap-no-sms-push-notifications',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
    ...
    console.error('[gap-no-sms-push-notifications] error:', err.message);
